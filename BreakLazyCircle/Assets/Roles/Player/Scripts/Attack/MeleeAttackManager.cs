...
public class MeleeAttackManager : MonoBehaviour
{
    ...
    //The CollisionSenses script on the player; this script on my project manages the grounded state, so if you have a different script for that reference that script
    ...
    private void CheckInput()
    {
        ...
        if (meleeAttack && Input.GetAxis("Vertical") < 0 && !collisionSenses.IsGround)
        {
        ...
        //OR if melee attack is true and pressing down while grounded
            || meleeAttack && (Input.GetAxis("Vertical") < 0 && collisionSenses.IsGround))
        {
