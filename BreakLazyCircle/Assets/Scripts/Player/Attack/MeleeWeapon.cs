...
public class MeleeWeapon : MonoBehaviour
{
    ...
    //Reference to CollisionSenses script which contains the value if the player is facing left or right; if you don't have this or it's named something different, either omit it or change the class name to what your CollisionSenses script is called
    ...
    private void Start()
    {
        //Reference to the CollisionSenses script on the player; if you don't have this or it's named something different, either omit it or change the class name to what your CollisionSenses script is called
        collisionSenses = GetComponentInParent<CollisionSenses>();
        //Reference to the Rigidbody2D on the player
    ...
    private void HandleCollision(EnemyHealth objHealth)
    {
        ...
        if (objHealth.giveUpwardForce && Input.GetAxis("Vertical") < 0 && !collisionSenses.IsGround)
        {
        ...
        if (Input.GetAxis("Vertical") > 0 && !collisionSenses.IsGround)
        {
            //Sets the direction variable to up
        ...
        if ((Input.GetAxis("Vertical") <= 0 && collisionSenses.IsGround) || Input.GetAxis("Vertical") == 0)
        {
            //Checks to see if the player is facing left; if you don't have a collisionSenses script, the commented out line of code can also check for that
